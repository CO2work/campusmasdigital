<template>
  <div ref="webgl" className="webgl"></div>
</template>
...
export default {
  ...
  methods: {
    ...
    initThree() {
      ...
      this.camera = new THREE.PerspectiveCamera(45, window.innerWidth / window.innerHeight, 1, 2);
      ...
      loader.load('/models/singapur.glb', (obj) => {
        //this.edificio_cu = obj;
        console.log("edificio", this.edificio_cu)
  ...
  watch: {
    '$route.path': function () {
      ...
      console.log(this.edificio_cu.position)
      this.camera.lookAt(this.edificio_cu.scene.position);
